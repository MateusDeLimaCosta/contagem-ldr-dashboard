...
def load_csv(file):
    # Leitura bruta para detectar delimitador
    ...
    # Agora ler com pandas usando o delimitador detectado
    df = pd.read_csv(
        io.StringIO(raw),
...
if uploaded is None:
    try:
        with open("contagem_ldr_202511181441.csv", "rb") as f:
            df = load_csv(f)
        use_default = True
    except:
        df = None
...
# Basic cleaning
# Normalizar nomes de coluna (Opcional, mas manter é bom)
# Normalizar colunas e remover aspas extras
df.columns = (
    df.columns
        .str.strip()
        .str.lower()
...
# --- NOVO TRECHO CRÍTICO DE CORREÇÃO ---
# Se as colunas existentes não corresponderem, force a renomeação.
# A melhor prática é apenas garantir que as colunas existam:
expected = ["id", "passagens", "timestamp"]
